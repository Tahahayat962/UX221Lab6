...
if ( is_singular( 'post' ) ) {
	...
	if ( $query->have_posts() ) :
		$related_title = get_theme_mod( 'terminal_blog_related_posts_title', __( 'Related Posts', 'terminal-blog' ) );
		?>
					...
					<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
						<div class="post-item post-grid">
							<div class="post-item-image">
								<a href="<?php the_permalink(); ?>"><?php terminal_blog_post_thumbnail(); ?></a>
							</div>
...
<?php endif; ?>
<?php
$terminal_blog_search = array( '[the-year]', '[site-link]' );
...
$copyright_text       = get_theme_mod( 'terminal_blog_copyright_txt', $copyright_default );
$copyright_text       = str_replace( $terminal_blog_search, $replace, $copyright_text );
?>
		...
		<div class="bottom-footer-info">
			<div class="site-info">
				<span>
					...
					<?php echo sprintf( esc_html__( 'Theme: %1$s By %2$s.', 'terminal-blog' ), wp_get_theme()->get( 'Name' ), '<a href="' . wp_get_theme()->get( 'AuthorURI' ) . '">' . wp_get_theme()->get( 'Author' ) . '</a>' ); ?>
				</span>	
			</div><!-- .site-info -->
		</div>
	</div>
