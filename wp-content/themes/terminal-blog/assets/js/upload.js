...
jQuery(function($){
  ...
  jQuery(document).on('click', 'input.select-img', function( event ){
    ...
    var Terminal_Blog_Image = wp.media.controller.Library.extend({
      defaults :  _.defaults({
    ...
    terminal_blog_file_frame = wp.media.frames.terminal_blog_file_frame = wp.media({
      button: {
        text: jQuery( this ).data( 'uploader_button_text' ),
      },
      state : 'author-info-insert-image',
      states : [
        new Terminal_Blog_Image()
        ],
          multiple: false  // Set to true to allow multiple files to be selected
        });

    // When an image is selected, run a callback.
    terminal_blog_file_frame.on( 'select', function() {

      var state = terminal_blog_file_frame.state('author-info-insert-image');
