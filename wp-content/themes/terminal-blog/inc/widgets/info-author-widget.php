<?php
if ( ! class_exists( 'Terminal_Blog_Author_Info_Widget' ) ) {
	/**
	 * Adds Terminal Blog Author Info Widget.
	 */
	class Terminal_Blog_Author_Info_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$terminal_blog_author_info_widget = array(
				'classname'   => 'widget adore-widget author-widget',
				'description' => __( 'Retrive Author Information Widget', 'terminal-blog' ),
			);
			parent::__construct(
				'terminal_blog_author_info_widget',
				__( 'Adore Widget: Author Info Widget', 'terminal-blog' ),
				$terminal_blog_author_info_widget
			);
		}
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'name' ) ); ?>"><?php esc_html_e( 'Author Name:', 'terminal-blog' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'designation' ) ); ?>"><?php esc_html_e( 'Author Designation:', 'terminal-blog' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'description' ) ); ?>"><?php esc_html_e( 'Description :', 'terminal-blog' ); ?></label>
			...
			<div>
				<label for="<?php echo esc_attr( $this->get_field_id( 'author_image_url' ) ); ?>"><?php esc_html_e( 'Author Image URL', 'terminal-blog' ); ?></label>:<br />
				...
				<div class="image-preview" style="<?php echo esc_attr( $style_attr ); ?>">
					<img src="<?php echo esc_url( $image_url ); ?>" alt="<?php esc_attr_e( 'Preview', 'terminal-blog' ); ?>" style="width: 200px;"/>
				</div><!-- .image-preview -->
				<input type="button" class="select-img button" value="<?php esc_attr_e( 'Upload', 'terminal-blog' ); ?>" data-uploader_title="<?php esc_attr_e( 'Select Image', 'terminal-blog' ); ?>" data-uploader_button_text="<?php esc_attr_e( 'Choose Image', 'terminal-blog' ); ?>" style="margin-top:5px;" /><br/>
			</div>
			<?php
			for ( $i = 1; $i <= 4; $i++ ) {
				...
				?>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'link' . '-' . $i ) ); ?>"><?php sprintf( esc_html__( 'Link %s :', 'terminal-blog' ), $i ); ?></label>
 ...
 *
 * @since Terminal Blog 1.0
 **/
function terminal_blog_author_info_widget_image_upload_enqueue( $hook ) {
	...
	wp_enqueue_media();
	wp_enqueue_script( 'author-info-widget-image-upload-script', get_template_directory_uri() . '/assets/js/upload.min.js', array( 'jquery' ), TERMINAL_BLOG_VERSION, true );
}

add_action( 'admin_enqueue_scripts', 'terminal_blog_author_info_widget_image_upload_enqueue' );
